...
from utils.variables import OPTION_OXIDATION_PARAMS, VariableFactory, TrialState
from utils.plotting import pareto_front
from fs.larkapi import LarkSheetSession

def app():
    st.markdown("# Human-in-the-loop(HITL) HMF Oxidation")
    
    st.sidebar.markdown("## Setup Experiment")

    exp_name = st.sidebar.text_input('Experiment Name', value="demo-oxidation")
    ...
    Y_vector = st.sidebar.multiselect(
        ...
        ['E-Factor', 'Productivity (mol/h)', 'Purity']
    )
    Y_dims = len(Y_vector)
    ...
    if fs_sheet_sync:
        ...
        if len(remote_df["Trial Index"].astype(int).tolist()) > 0:
